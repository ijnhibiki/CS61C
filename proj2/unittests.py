...
class TestRelu(unittest.TestCase):
    ...
    def test_relu_large(self):
        ...
        t = AssemblyTest(self, "relu.s")
        # create an array in the data section
        array0 = t.array([-23221, -2672726, 378373, 28928, -1, -3, 78, 1234567, 2147483641, -2147483641])
        # load address of `array0` into register a0
        t.input_array("a0", array0)
        # set a1 to the length of our array
        t.input_scalar("a1", len(array0))
        ...
        t.call("relu")
        # check that the array0 was changed appropriately
        t.check_array(array0, [0, 0, 378373, 28928, 0, 0, 78, 1234567, 2147483641, 0])
        # generate the `assembly/TestRelu_test_simple.s` file and run it through venus
        t.execute()
...
class TestArgmax(unittest.TestCase):
    ...
    def test_last_biggest_element(self):
        t = AssemblyTest(self, "argmax.s")
        array0 = t.array([3, 4, 4, 5, 6, 10, -1, 17])
        t.input_array("a0", array0)
        # set a1 to the length of the array
        t.input_scalar("a1", len(array0))
        # call the `argmax` function
    ...
    def test_multiple_biggest(self):
        t = AssemblyTest(self, "argmax.s")
        array0 = t.array([3, 17, 17, 5, 6, 10, -1, 17])
        t.input_array("a0", array0)
        # set a1 to the length of the array
        t.input_scalar("a1", len(array0))
        # call the `argmax` function
